...
class TestEvaluateSingleAttack:
    ...
    def test_known_3v2_probabilities(self):
        # Standard Risk 3v2 probabilities (out of 7776 total permutations)
        outcomes = {(a, d): p for a, d, p in evaluate_single_attack(3, 2)}
        assert abs(outcomes[(-2, 0)] - 2275 / 7776) < 1e-9   # defender wins both
        assert abs(outcomes[(-1, -1)] - 2611 / 7776) < 1e-9  # split
        assert abs(outcomes[(0, -2)] - 2890 / 7776) < 1e-9   # attacker wins both
